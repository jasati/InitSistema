(function() {
    ...
    EquipeFuncService.$inject = [
      'UtilsFunctions','EccDataset','UtilsDataFunctionService','FiltroService','EquipeCasaisFuncService','EncontreirosFuncService','EquipesReport',
        ...
        function funcoes(idEncontro) {
          ...
          var showReport = function () {
            var config = {
              ...
              ariaDescribedBy: 'equipe-modal',
              size:'',
              data:vm.equipe.report,
              backdrop:'static',
              fullscreen:false,
              modal:{},
            };
            vm.equipe.showModal(config);
          }
          vm.report = function () {
            vm.equipeCasais.equipeCasais.read(' and e.id_encontro = '+idEncontro).then(function (result) {
              vm.equipe.report.template = EquipesReport.listagem(vm.equipe.rows,vm.equipeCasais.equipeCasais.rows);
              vm.equipe.report.title = 'Visualizar impressão das equipes';
              vm.equipe.report.orientacaoPage = 'L';
              vm.equipe.report.createPdf().then(function (result) {
                if (result) {
                  showReport();
                }
              });
