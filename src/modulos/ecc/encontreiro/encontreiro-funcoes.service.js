(function() {
    ...
    EncontreirosFuncService.$inject = [
      'UtilsFunctions','EccDataset','UtilsDataFunctionService','FiltroService','CasaisFuncService','EncontristaFuncService','EncontreiroReport',
      '$mdDialog','$filter'
    ];
        ...
        function funcoes(idEncontro) {
          ...
          var showReport = function () {
            var config = {
              ...
              ariaLabelledBy: 'Visualizar impressão de encontreiros',
              ariaDescribedBy: 'encontreiro-modal',
              size:'',
              data:vm.encontreiros.report,
              backdrop:'static',
              fullscreen:false,
              modal:{},
            };
            vm.encontreiros.showModal(config);
          }

          vm.filtrar = function () {
            var query = ' and ce.id_encontro = '+idEncontro;
            if (isset(vm.encontreiros.filtros.mainField)) {
              query += " and (CASE WHEN c.id_conjuge_marido is null or (c.id_conjuge_marido = 0) THEN CONCAT(c.marido,c.esposa) ELSE CONCAT(cm.nome_red,ces.nome_red) END)  LIKE '%"+vm.encontreiros.filtros.mainField+"%'";
            }
          ...
          vm.report = function () {
            vm.encontreiros.report.template = EncontreiroReport.listagem(vm.encontreiros.rows);
            vm.encontreiros.report.title = 'Visualizar impressão de encontreiros';
            vm.encontreiros.report.orientacaoPage = 'L';
            vm.encontreiros.report.createPdf().then(function (result) {
              if (result) {
                showReport();
              }
            });
